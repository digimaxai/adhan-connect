-- Silent additive rollout for prayer_times and staff_rota
-- No existing tables or policies are modified. All changes are additive and idempotent.
...
-- Enable RLS (additive; safe if already enabled) ---------------------------
alter table if exists prayer_times enable row level security;
alter table if exists staff_rota enable row level security;

-- Helper: local admin via mosque_admins ------------------------------------
-- Policies are added only if they do not already exist to remain idempotent.
