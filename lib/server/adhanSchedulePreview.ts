...
import { ADHAN_PRAYERS, AdhanAudioAsset, AdhanAudioDraft, AdhanAudioError, EMPTY_ADHAN_AUDIO_DRAFT, validateAudioDraft } from '../adhanAudio.ts';
import { AdhanPlanSlot, AdhanSchedulePreview, explicitInstant, mosqueLocalDate, nextCalendarDate, planAdhanDay } from '../adhanDelivery.ts';

const ADHAN_COLUMNS = ADHAN_PRAYERS.map(p => `${p}_adhan_time`).join(',');

export type MosqueLike = { id: string; status: string; time_zone: string | null; prayers_not_offered: string[] | null };
// Deliberately not importing AdminAccessContext from ./adminAccess: that file
// (and its own ./accountConsentAccess import) pulls in a much larger
// dependency tree unrelated to schedule resolution, and this module is also
// loaded directly by the Deno adhan-delivery-plan Edge Function, which
// resolves every import (including type-only ones) while building its
// module graph. This structural type is all either caller actually needs.
type MinimalDb = { from: (table: string) => any };

/** Shared prayer-time resolution used by both the admin preview (draft, never
 * activated) and the delivery planner (only ever called for activated
 * mosques). Reused so both paths can never silently diverge on what "today's
 * resolved prayer time" means. Never writes; a pure read + pure planAdhanDay. */
export async function resolveAdhanScheduleSlots(
  db: MinimalDb,
  mosque: MosqueLike,
  ...
  now: string
): Promise<AdhanPlanSlot[]> {
  const timeZone = mosque.time_zone?.trim() ?? '';
  let today: string;
  try { today = mosqueLocalDate(now, timeZone); }
  catch { throw new AdhanAudioError('Set a valid mosque timezone in Mosque Profile before scheduling automatic playback.', 409); }
  const slots: AdhanPlanSlot[] = [];
  for (const date of [today, nextCalendarDate(today)]) {
    const query = new URLSearchParams({ mosqueId: mosque.id, date });
    ...
    if (!response.ok && mosque.status === 'active') throw new AdhanAudioError('Prayer times could not be loaded. Try again later.', 503);
    ...
    if (timeZone !== 'Europe/London') {
      ...
      const canonical = await db.from('prayer_times').select(ADHAN_COLUMNS).eq('mosque_id', mosque.id).eq('date', date).maybeSingle() as
        { data: Record<string, string | null> | null; error: { message: string } | null };
    ...
    slots.push(...planAdhanDay({
      mosqueId: mosque.id, localDate: date, timeZone, mosqueActive: mosque.status === 'active',
      notOffered: mosque.prayers_not_offered ?? [], settings, assets,
...
export async function loadAdhanSchedulePreview(
  context: { supabaseAdmin: MinimalDb },
  mosqueId: string,
  ...
  return { automaticPlaybackActive: false, timeZone: mosque.time_zone?.trim() ?? '', generatedAt: now, settingsRevision: settings.revision, slots };
}
