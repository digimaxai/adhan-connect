...
export const GET: RequestHandler = async (request) => {
  ...
  // Main-admin authority is represented by `isMainAdmin`; it does not require
  // serialising the entire mosque directory into the login/route-guard payload.
  // Main-admin pages already page/query that directory independently. Keep only
  // explicit local-admin memberships here for lightweight workspace context.
  const adminMosques = membershipAdminMosques;
  ...
  return json({
    ...
    isLocalAdmin: membershipAdminMosques.length > 0,
