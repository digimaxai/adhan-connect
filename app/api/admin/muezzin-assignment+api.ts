...
async function requireMainAdmin(
  ...
  if (requesterError || !requesterProfile || requesterProfile.role !== 'main_admin') {
    return { response: json({ error: 'Only main admin users can manage muezzin assignments.' }, 403) };
  }
...
export const POST: RequestHandler = async (request) => {
  ...
  const { data: existingRow, error: existingError } = await (supabaseAdmin as SupabaseClient<any, any, any>)
    .from('muezzins')
    .select('user_id, mosque_id, is_active')
    .eq('user_id', userId)
    .eq('mosque_id', mosqueId)
    .maybeSingle();

  if (existingError) {
    return json({ error: existingError.message || 'Unable to inspect the current muezzin assignment.' }, 500);
  }

  if (existingRow) {
    if (existingRow.is_active === false) {
      const { error: reactivateError } = await (supabaseAdmin as SupabaseClient<any, any, any>)
        .from('muezzins')
      ...
      if (reactivateError) {
        return json({ error: reactivateError.message || 'Unable to reactivate this muezzin assignment.' }, 500);
      }

      return json({ userId, mosqueId, assigned: true, reactivated: true });
    }

    return json({ error: 'That user already has muezzin access to this mosque.' }, 409);
  }
  ...
  if (error) {
    const message = error.message || 'Unable to assign this muezzin.';
...
export const DELETE: RequestHandler = async (request) => {
  ...
  if (error) {
    return json({ error: error.message || 'Unable to remove this muezzin assignment.' }, 500);
  }

  if (!data || data.length === 0) {
    return json({ error: 'This muezzin assignment no longer exists.' }, 404);
  }
