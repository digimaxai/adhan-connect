...
export const POST: RequestHandler = async (request) => {
  ...
  if (!supabaseUrl && !serviceRoleKey) {
    return json(
      {
        error:
          'Server is missing both SUPABASE_URL/EXPO_PUBLIC_SUPABASE_URL and SUPABASE_SERVICE_ROLE.',
      },
      500
    );
  }
  ...
  if (!supabaseUrl) {
    return json(
      {
        error: 'Server is missing SUPABASE_URL or EXPO_PUBLIC_SUPABASE_URL.',
      },
      500
    );
  }
  ...
  if (requesterError || !requesterProfile || requesterProfile.role !== 'main_admin') {
    return json({ error: 'Only main admin users can invite or assign local admins.' }, 403);
  }
  ...
  if (saveProfileError || !savedProfile) {
    return json({ error: 'Unable to persist the local admin profile.' }, 500);
  }
