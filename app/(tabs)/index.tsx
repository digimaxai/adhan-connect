...
export default function HomeScreen() {
  ...
  const LiveCard = () => {
    const liveFromSubs = Object.entries(liveStreams).find(([mosqueId]) => subscribedIds.has(mosqueId));
    const fallbackLive = Object.entries(liveStreams)[0];
    const heroLive = liveFromSubs ?? fallbackLive;
    if (!heroLive) return null;

    const [liveMosqueId] = heroLive;
    const liveMosque = mosques.find((m) => m.id === liveMosqueId);
    const liveTitle = liveMosque?.name ?? 'Live stream';
    const liveCity = [liveMosque?.city, liveMosque?.country].filter(Boolean).join(', ');
    ...
    return (
      <View style={[styles.liveCard, styles.shadow]}>
        <View style={styles.liveRow}>
          ...
          <View style={{ flex: 1, gap: 4 }}>
            <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
              ...
              <Text style={styles.liveSmall}>{liveCity || 'Broadcasting Adhan'}</Text>
            </View>
        ...
        <Pressable
          ...
          style={({ pressed }) => [styles.liveButtonPrimary, { opacity: pressed ? 0.9 : 1, marginTop: 14 }]}
        >
          <Text style={styles.liveButtonPrimaryText}>Listen Live</Text>
        </Pressable>
      </View>
    );
  };
  ...
  const MyMosques = () => (
    <View style={[styles.cardContainer, styles.shadow]}>
      ...
      {subs.length ? (
        <ScrollView horizontal showsHorizontalScrollIndicator={false} contentContainerStyle={{ gap: 14, paddingTop: 10, paddingBottom: 4 }}>
          {subs
    ...
    <SafeAreaView style={styles.screen} edges={['top', 'left', 'right']}>
      <ScrollView contentContainerStyle={[styles.scrollBody, { paddingTop: topPad + 12 }]}>
        ...
        >
          ...
          <Text style={styles.nextName}>{nextPrayer?.name ? nextPrayer.name.charAt(0).toUpperCase() + nextPrayer.name.slice(1) : 'Fajr'}</Text>
          <Text style={styles.nextEta}>{nextPrayer?.remaining ? `In ${nextPrayer.remaining}` : 'In 06:49'}</Text>
          {primaryMosque?.name ? <Text style={styles.sourceMuted}>{primaryMosque.name}</Text> : null}
        </Pressable>
        ...
        <View style={[styles.cardContainer, styles.shadow]}>
          <View style={styles.sectionHeader}>
            ...
            {primaryMosque?.name ? <Text style={styles.sourceText}>{primaryMosque.name}</Text> : null}
          </View>
        ...
        <View style={[styles.discoveryCard, styles.shadow]}>
          ...
          <Text style={styles.cardSubtitle}>Listen to live adhans and follow your favorites.</Text>
          <Pressable
            onPress={() => router.push('/(tabs)/discover')}
            style={({ pressed }) => [styles.discoveryBtn, { opacity: pressed ? 0.9 : 1, marginTop: 12 }]}
          >
...
const styles = StyleSheet.create({
  ...
  scrollBody: { paddingHorizontal: 16, paddingBottom: 32, gap: 16 },
  headerRow: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  appTitle: { fontSize: 24, fontWeight: '800', letterSpacing: 0.2, color: '#0F172A' },

  eyebrow: { color: '#0EA5E9', fontWeight: '700', fontSize: 12, letterSpacing: 0.5 },
  nextCard: { backgroundColor: '#0E172A', borderRadius: 16, padding: 16 },
  nextTime: { color: '#FFFFFF', fontSize: 34, fontWeight: '900', marginTop: 6 },
  nextName: { color: '#B4E0FF', fontSize: 18, fontWeight: '800', marginTop: 2 },
  nextEta: { color: '#7EE0A3', fontSize: 13, fontWeight: '700', marginTop: 4 },
  sourceMuted: { color: '#CBD5E1', fontSize: 12, marginTop: 6, fontWeight: '600' },

  liveCard: { backgroundColor: '#FFF4F4', borderRadius: 16, padding: 16, gap: 10 },
  liveRow: { flexDirection: 'row', gap: 12, alignItems: 'center' },
  liveIconWrap: { padding: 8, borderRadius: 12, backgroundColor: '#FFECEC' },
  ...
  liveButtonPrimaryText: { color: '#FFFFFF', fontWeight: '800', fontSize: 14 },

  cardContainer: { backgroundColor: '#FFFFFF', borderRadius: 16, padding: 16 },
  sectionHeader: { flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between' },
  cardTitle: { fontSize: 17, fontWeight: '800', color: '#0F172A' },
  cardSubtitle: { color: '#64748B', fontSize: 13 },
  manageLink: { color: '#0EA5E9', fontWeight: '800', fontSize: 13 },

  mosqueChip: { alignItems: 'center', width: 110 },
  mosqueAvatar: {
    width: 64,
    height: 64,
    borderRadius: 32,
  ...
  mosqueAvatarText: { fontWeight: '800', color: '#0369A1', fontSize: 16 },
  mosqueLabel: { marginTop: 8, fontWeight: '700', fontSize: 13, color: '#0F172A' },
  mosqueCity: { color: '#94A3B8', fontSize: 12, marginTop: 2 },

  prayerTable: { marginTop: 8, gap: 8 },
  prayerRow: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', paddingVertical: 6 },
  prayerName: { fontWeight: '800', color: '#0F172A', letterSpacing: 0.3 },
  ...
  discoveryCard: { backgroundColor: '#FFFFFF', borderRadius: 16, padding: 16, marginBottom: 8 },
  discoveryBtn: {
    backgroundColor: '#0EA5E9',
    paddingVertical: 12,
