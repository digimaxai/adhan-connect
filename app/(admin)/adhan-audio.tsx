...
import {
  ADHAN_PRAYERS, AdhanAudioDraft, AdhanAudioMode, AdhanAudioWorkspace, validateAdhanSetupDraft,
} from '../../lib/adhanAudio';
...
const MODES: { value: AdhanAudioMode; title: string; description: string }[] = [
  ...
  { value: 'recorded_only', title: 'Scheduled recording only', description: 'Prepare a recording for the mosque’s adhan times. Scheduled playback is coming later.' },
];
...
function AdhanAudioWorkspaceScreen() {
  ...
  return <ScreenContainer contentStyle={styles.page} keyboardShouldPersistTaps="handled">
    ...
    <View style={styles.notice}>
      <AppText style={styles.title}>Recording preparation</AppText>
      <AppText>Upload and preview recordings, choose a planned mode and save your preferences. Scheduled playback is not available yet.</AppText>
    </View>
  ...
  return <>
    ...
    {workspace && draft ? <>
      <View style={styles.card}>
        ...
        {draft.draft_mode === 'live_with_fallback' ? <AppText style={styles.muted}>
          Recording fallback is deferred. Choose Live only or Scheduled recording only before saving.
        </AppText> : null}
        {workspace.active ? <AppText style={styles.error}>
          An earlier scheduling test is active. The central team must safely pause it before these settings can be changed.
        </AppText> : null}
        ...
        <Choice title="Save preparation settings" disabled={busy || workspace.active || draft.draft_mode === 'live_with_fallback'} onPress={() => void run(async () => {
          const settings = validateAdhanSetupDraft(draft);
          ...
          if (alive.current) {
            ...
            setMessage('Preparation settings saved. Scheduled playback is not available yet.');
          }
        })} />
      </View>
      <View style={styles.card}>
        ...
        <Choice title="Check saved schedule" disabled={busy || draft.draft_mode === 'live_with_fallback'} onPress={() => void run(async () => {
          const result = await adhanAudioRequest<AdhanSchedulePreview>(token, mosqueId, { action: 'preview_schedule', settings: validateAdhanSetupDraft(draft) });
